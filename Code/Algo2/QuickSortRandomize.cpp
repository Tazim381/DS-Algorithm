...
int updatedrandomizepartition(vector<int>&arr, int l , int r)
{
    int ran = l+(rand()*rand())%(r-l+1);
    swap(arr[ran],arr[r]);
...
void updatedrandomizequicksort(vector<int>&arr,int l, int r)
{
    if(l<r)
    {
        int p = updatedrandomizepartition(arr,l,r);
        updatedrandomizequicksort(arr,l,p-1);
        updatedrandomizequicksort(arr,p+1,r);
    }
}
...
//    }
    ...
    cout<<"time needs for quickort = "<<double(t4-t3)/CLOCKS_PER_SEC<<endl;
     ...
     t5 = clock();
     updatedrandomizequicksort(ar,0,n-1);
     t6 = clock();
     cout<<"time needs for updatedrandomizequickort = "<<double(t6-t5)/CLOCKS_PER_SEC<<endl;
